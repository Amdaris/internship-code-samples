...
var panFryingTask = PanFrying();
var toastTask = MakeToastAsync();

await Task.WhenAll(coffeeTask, panFryingTask, toastTask);
...
static async Task PanFrying()
{
    await HeatPanAsync();
    var fryingEgssTask = FryEggsAsync();
    var fryingBaconTask = FryBaconAsync();
    await Task.WhenAll(fryingEgssTask, fryingBaconTask);
}
